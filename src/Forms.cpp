...
void SidePanel::finalize( Game* game ) {
    SAFE_RELEASE( _form );
}
...
void SidePanel::render( Game* game, float elapsedTime ) {
    _form->draw();
}
    ...
    switch ( evt ) {
        ...
        case Control::Listener::VALUE_CHANGED:
            if ( control == _sliderLightVectorX ||
                 control == _sliderLightVectorY ||
                 control == _sliderLightVectorZ ) {
                Vector3 lightVector( _sliderLightVectorX->getValue(), 
                                     _sliderLightVectorY->getValue(),
                                     _sliderLightVectorZ->getValue());
                delegate->setLightVector( lightVector );
            }
            if ( control == _sliderClearColorRed ||
                 control == _sliderClearColorGreen ||
                 control == _sliderClearColorBlue ) {
                Vector4 clearColor ( _sliderClearColorRed->getValue(),
                                     _sliderClearColorGreen->getValue(),
